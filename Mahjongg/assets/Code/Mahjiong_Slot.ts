...
export class Mahjiong_Slot extends Component {
    ...
    Clear_OK(x) {//消除成功回调函数
        ...
        } else if (x == 2) {//相同麻将不足3个
            this.node.getComponent(Mahjiong_SlotMax).Max(this.Node_List.length)//判断卡槽上限
        } else if (x == 3) {//如果是三消
    ...
    */
    ...
    Set_Order(node: Node) {//设置排列顺序
        ...
        let num = node.getComponent(Mahjiong_Prefab).Num//获取新麻将的编号
        for (let i = list.length - 1; i >= 0; i--) {//倒序遍历列表
            //倒序可以保证新麻将永远在相同麻将后面
            //如果是正序，遇到相同的麻将就可能会从中间插入
            if (num == list[i].getComponent(Mahjiong_Prefab).Num) {//如果新麻将编号与当前麻将编号相同
